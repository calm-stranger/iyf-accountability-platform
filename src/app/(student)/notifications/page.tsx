...
import { Bell, CheckCheck, Trophy, MessageSquare, FileText, Info } from 'lucide-react'
...
function NotifIcon({ type }: { type: string }) {
  ...
  return <Info size={15} className="text-muted-foreground" />
}
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-64">
      <div className="text-center"><div className="text-3xl mb-2 animate-pulse">🪷</div>
        <p className="text-sm text-muted-foreground">Loading notifications...</p></div>
    </div>
  )
                ...
                <div className="flex gap-3 items-start">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 justify-between">
                      <p className={`text-sm font-medium ${!n.is_read ? 'text-foreground' : 'text-foreground/80'}`}>
                        {n.title}
                      </p>
